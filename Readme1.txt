...
A basic CNN color constancy architecture was implemented in assignment 1, in assignment 2 the architecture was further refined through hyperparameter tuning and experimental testing. The network is trained and run on the color constancy dataset with 17 different sets of parameters combinations (activation functions, dropout rates, filter sizes etc.), additionally a standalone GAN is trained and run on the dataset for comparison, and finally a hybrid approach is used where the CNN output is run through the GAN for further refinement.  

Instructions to run the files:

The project consists of eight CNN files and 5 GAN files:
...
1. dataloader.py: This script handles data loading for the GAN. It includes functions to load .tiff or .tif images from a specified folder and load illuminant values from an Excel file. It resizes images to 256x256 pixels and converts them to tensors for further processing.

2. network.py: Defines the GAN's architecture, which includes both the generator and discriminator models. The generator uses ResNet blocks to generate color-corrected images, while the discriminator distinguishes between real and fake images. The generator and discriminator are trained adversarially to improve the accuracy of the generated images.

3. preprocessing.py: Contains preprocessing functions used by the GAN. This includes resizing images to dimensions that are multiples of 32, extracting 32x32 patches, and applying histogram stretching for contrast normalization. These preprocessing steps prepare the images for GAN training, ensuring consistent input quality.

4. test.py: This script is used to test the trained GAN model. It takes an input image, applies the generator to produce a white-balanced output, and saves the result. It also provides functionality to display both the input and generated images side-by-side for easy comparison.

5. train.py: This file is responsible for training the GAN model. It uses images and illuminant data, loaded from specified paths, and iteratively trains both the generator and discriminator. The model is optimized using Mean Squared Error (MSE) loss, and the trained generator is saved to a .pth file for future use.
